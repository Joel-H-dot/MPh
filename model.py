...
class Model:
    ...
    #     self.java.study().
    #     self.java.feature('freq').set('plist', str(frequency))
    ...
    def interp(self, expression, coord=None, unit=None, dataset=None,
        ...
        if dtype == 'particle':
            ...
            if eval.isComplex():
                ...
                # import pandas as pd
                # data_frame = pd.DataFrame(eval.getReal())
                # data_frame.to_excel(r'C:\Users\EEE Admin\Desktop\ALL CODE\FORWARD\real.xlsx')
                # data_frame = pd.DataFrame(eval.getImag())
                # data_frame.to_excel(r'C:\Users\EEE Admin\Desktop\ALL CODE\FORWARD\imag.xlsx')

                results = numpy.zeros(D.shape,dtype=numpy.complex_)
                results = D
            if inner is None:
                pass
            elif inner == 'first':
    ...
    def evaluate(self,  expression, coord=None, unit=None, dataset=None,
                       inner=None, outer=None):
        """
